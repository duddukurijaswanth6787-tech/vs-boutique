// Core Components — Shared UI primitives, layouts, navigation, feedback, and animations.
// All exports are re-exported from their current locations. Files will be moved in Phase 2.

// UI Primitives
export { default as Accordion } from './ui/Accordion';
export { default as Avatar } from './ui/Avatar';
export { default as Badge } from './ui/Badge';
export { default as BottomSheet } from './ui/BottomSheet';
export { default as Button } from './ui/Button';
export { default as Card } from './ui/Card';
export { default as Checkbox } from './ui/Checkbox';
export { default as Chip } from './ui/Chip';
export { default as Drawer } from './ui/Drawer';
export { default as EmptyState } from './ui/EmptyState';
export { default as ErrorState } from './ui/ErrorState';
export { default as FAB } from './ui/FAB';
export { default as IconButton } from './ui/IconButton';
export { default as Input } from './ui/Input';
export { default as LoadingOverlay } from './ui/LoadingOverlay';
export { default as Modal } from './ui/Modal';
export { default as PremiumImage } from './ui/PremiumImage';
export { default as Radio } from './ui/Radio';
export { default as SearchInput } from './ui/SearchInput';
export { default as Select } from './ui/Select';
export { default as Skeleton, TableSkeleton } from './ui/Skeleton';
export { default as Stepper } from './ui/Stepper';
export { default as Switch } from './ui/Switch';
export { default as Tabs } from './ui/Tabs';
export { default as Textarea } from './ui/Textarea';
export { default as Timeline } from './ui/Timeline';
export { default as Toast } from './ui/Toast';
export { default as Tooltip } from './ui/Tooltip';

// Layout Primitives
export { default as Container } from './layout/Container';
export { default as Grid } from './layout/Grid';
export { default as PageFooter } from './layout/PageFooter';
export { default as PageHeader } from './layout/PageHeader';
export { default as Section } from './layout/Section';
export { default as Stack } from './layout/Stack';

// Navigation
export { default as Sidebar } from './navigation/Sidebar';
export { default as Navbar } from './navigation/Navbar';
export { default as MegaMenu } from './navigation/MegaMenu';
export { default as MobileNavSheet } from './navigation/MobileNavSheet';

// Shared / Cross-portal
export { default as LegalPage } from './shared/LegalPage';
export { default as PremiumLegalPage } from './shared/PremiumLegalPage';
export { default as AppPreviewMockup } from './shared/AppPreviewMockup';
export { default as DeleteConfirm } from './shared/DeleteConfirm';
export { default as EditModal } from './shared/EditModal';
export { default as AddressFormModal } from './shared/AddressFormModal';
export { default as ExchangeRequestModal } from './shared/ExchangeRequestModal';
export { default as OtpModal } from './shared/OtpModal';
export { default as ReturnRequestModal } from './shared/ReturnRequestModal';
export { default as ReviewModal } from './shared/ReviewModal';
export { default as BoutiqueEditModal } from './shared/BoutiqueEditModal';
export { default as BoutiqueForm } from './shared/BoutiqueForm';
export { default as BoutiqueTable } from './shared/BoutiqueTable';
export { default as CategoryList } from './shared/CategoryList';
export { default as AccessControlTab } from './boutique/AccessControlTab';
export { default as BoutiqueProfileTab } from './boutique/BoutiqueProfileTab';
export { default as DangerZoneTab } from './boutique/DangerZoneTab';
export { default as OwnerDetailsTab } from './boutique/OwnerDetailsTab';

// Commerce Components (shared across customer + admin portals)
export { default as AvailabilityBadge } from './commerce/AvailabilityBadge';
export { default as BoutiqueCard } from './commerce/BoutiqueCard';
export { default as CollectionCard } from './commerce/CollectionCard';
export { default as DeliveryBadge } from './commerce/DeliveryBadge';
export { default as DiscountBadge } from './commerce/DiscountBadge';
export { default as PriceComponent } from './commerce/PriceComponent';
export { default as CommerceProductCard } from './commerce/ProductCard';
export { default as RatingComponent } from './commerce/RatingComponent';
export { default as CommerceReviewCard } from './commerce/ReviewCard';
export { default as ServiceCard } from './commerce/ServiceCard';
export { default as StatusBadge } from './commerce/StatusBadge';
export { default as TimelineCard } from './commerce/TimelineCard';
