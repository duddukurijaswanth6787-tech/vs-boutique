...
import {
  Search, Truck, ChevronLeft, ChevronRight, Plus, X, Loader2,
  Package, MapPin, CheckCircle, Clock, AlertTriangle
} from 'lucide-react';
...
const AdminDeliveryTracking = () => {
  ...
  const { data: fetchedTracking, isLoading: trackingLoading, isError: trackingError } = useQuery({
    queryKey: ['order-tracking', expandedOrder],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['order-tracking', expandedOrder]);
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      {isLoading ? (
        ...
        <>
          {/* Mobile + Desktop: Card-based accordion list */}
