...
import {
  getOwnerCoupons, createOwnerCoupon, updateOwnerCoupon,
  toggleOwnerCoupon, deleteOwnerCoupon
} from '@core/services';
import OwnerLayout from '../../../../components/OwnerLayout';
import { TableSkeleton } from '@core/components/ui/Skeleton';
import { Plus, X, Search, Edit2, Trash2, Power, PowerOff, Tag, Calendar, AlertTriangle, Loader2 } from 'lucide-react';
...
const OwnerCoupons = () => {
  ...
  const { data: coupons = [], isLoading, error } = useQuery({
    ...
    queryFn: getOwnerCoupons,
  });

  const createMutation = useMutation({
    mutationFn: createOwnerCoupon,
    onSuccess: () => { queryClient.invalidateQueries(['owner-coupons']); setShowForm(false); resetForm(); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => updateOwnerCoupon(id, data),
    onSuccess: () => { queryClient.invalidateQueries(['owner-coupons']); setShowForm(false); resetForm(); },
  });

  const toggleMutation = useMutation({
    mutationFn: toggleOwnerCoupon,
    onSuccess: () => queryClient.invalidateQueries(['owner-coupons']),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteOwnerCoupon,
    onSuccess: () => { queryClient.invalidateQueries(['owner-coupons']); setIsDeleteModalOpen(false); },
  });
      ...
      <div className="space-y-8 pb-20">
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 rounded-[2.5rem] p-8 text-center">
            <p className="text-red-500 font-bold">Failed to load coupons.</p>
