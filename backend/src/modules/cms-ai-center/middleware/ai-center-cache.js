...
const CACHE_TTL = parseInt(process.env.AI_CENTER_CACHE_TTL || '300', 10);
...
async function set(key, data, ttl = CACHE_TTL) {
  const client = getClient();
  if (!client) return;
  try { await client.setex(key, ttl, JSON.stringify(data)); } catch (e) { console.error('[ai-center Cache] set error:', e); }
}

async function del(pattern) {
  const client = getClient();
  if (!client) return;
  try { const keys = await client.keys(pattern); if (keys.length > 0) await client.del(...keys); } catch (e) { console.error('[ai-center Cache] delPattern error:', e); }
}
